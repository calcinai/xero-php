...
use XeroPHP\Remote\OAuth\XeroException;
...
class RSASHA1 implements SignatureMethodInterface {
    ...
    public static function generateSignature(array $config, $sbs, $secret) {
        ...
        if(false === $private_key_id = openssl_pkey_get_private($config['rsa_private_key'])){
            throw new XeroException("Cannot access private key for signing: openssl_pkey_get_private('${config['rsa_private_key']}') returned false");
        }

        // Sign using the key
        if(false === openssl_sign($sbs, $signature, $private_key_id)) {
            throw new XeroException('Cannot sign signature base string.');
        }
